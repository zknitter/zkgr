...
#'
#' This function launches a browser window for the `zkgr` package
...
docs <- function(){
  ...
  if(is_AA()){
    ...
    utils::browseURL(pkgdown_doc_index(package = "zkgr"))
  }
}
...
serve_docs <- function(){
  ...
  if(is_AA()){
    ...
    servr::httd(pkgdown_doc_root(package = "zkgr"), verbose = FALSE)
  }
}
